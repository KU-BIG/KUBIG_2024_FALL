...
def compute_weighted_kl_dco_loss(loss_model, loss_refer, timesteps, beta):
    timesteps = timesteps.float()

    sigma = 0.1  # hyperparam
    w_t = torch.exp(-((timesteps - timesteps.mean()) ** 2) / (2 * sigma ** 2))

    diff = torch.clamp(loss_model - loss_refer, min=-1, max=1)

    integral_term = torch.sum(w_t * diff, dim=0)
    
    epsilon = 1e-5  # for preventingo log 0
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                try:
                    ...
                    if args.dcoloss > 0.0:
                        dco_loss = compute_weighted_kl_dco_loss(loss_model, loss_refer, timesteps, args.dcoloss)
                        loss = dco_loss
                    else:
                        loss = loss_model
