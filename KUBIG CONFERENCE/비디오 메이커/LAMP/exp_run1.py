...
# added lost_lass in line 323, 324 to preserve consistency better
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                latents = vae.encode(pixel_values).latent_dist.sample().to(weight_dtype) 
                latents = rearrange(latents, "(b f) c h w -> b c f h w", f=video_length)
                latents = latents * 0.18215

                # 노이즈 샘플 생성
                noise = torch.randn_like(latents, dtype=weight_dtype)  # dtype 명시
                bsz = latents.shape[0]

                # 타임스텝 생성
                timesteps = torch.randint(0, noise_scheduler.num_train_timesteps, (bsz,), device=latents.device, dtype=torch.long)

                # 타임스텝에 따라 노이즈 추가
                noisy_latents = noise_scheduler.add_noise(latents, noise, timesteps).to(weight_dtype)  # dtype 일치

                # 텍스트 임베딩 생성
                encoder_hidden_states = text_encoder(batch["prompt_ids"])[0].to(weight_dtype)  # dtype 일치


                if noise_scheduler.prediction_type == "epsilon":
                    target = noise
                elif noise_scheduler.prediction_type == "v_prediction":
                    target = noise_scheduler.get_velocity(latents, noise, timesteps).to(weight_dtype)
                else:
                ...
                model_pred = unet(noisy_latents, timesteps, encoder_hidden_states).sample.to(weight_dtype)  # dtype 일치
                loss_main = F.mse_loss(
                    model_pred[:, :, 1:, :, :],
                    target[:, :, 1:, :, :],
                    reduction="mean"
                )
                last_pred = model_pred[:, :, -1:, :, :]
                last_targ = target[:, :, -1:, :, :]

                loss_last = F.mse_loss(last_pred, last_targ, reduction="mean")
                loss = loss_main + 0.2 * loss_last
